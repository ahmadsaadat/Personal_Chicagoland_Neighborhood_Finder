...
tiled with no gaps and no overlaps anywhere in the modeled region. This was
generated by a one-time, dev-only script, `scripts/generate-tessellation.mjs`
(run via `npm run geo:generate`), using two devDependencies-only libraries,
`d3-delaunay` and `@turf/turf`. Every suburb feature was tagged
`properties.geometrySource: "voronoi-illustrative"` so it was never confused
with a real municipal boundary. **This was superseded in round 3 — see
below.**

**Update (round 3):** a user compared the shipped map against Google Maps and
confirmed the real Chicago polygons look correct, but caught that the round-2
Voronoi suburb shapes clearly didn't match reality — specifically calling out
Cicero and Berwyn, both narrow, elongated towns whose real shape a "closest
centroid wins" Voronoi cell has no way to reproduce.

All 102 Voronoi cells were replaced with real municipal boundary geometry
from the U.S. Census Bureau's **TIGER/Line "Places"** dataset, fetched live
from the Census TIGERweb ArcGIS REST service
(`Generalized_ACS2024/Places_CouSub_ConCity_SubMCD`, the "Incorporated Places
500K" layer) and matched to each suburb by name (with real-centroid
disambiguation for the couple of names, like "Wilmington," that recur
elsewhere in Illinois). All 102 suburbs matched successfully — none needed a
fallback. `properties.geometrySource` is now `"official"` for all 179
features (77 Chicago + 102 suburbs); `"voronoi-illustrative"` is retained
only as an escape hatch (see `docs/DATA_SOURCES.md`) if a future new suburb
can't be matched to a real Census Place.

The old `scripts/generate-tessellation.mjs` was replaced by
`scripts/generate-geometry.mjs` (still run via `npm run geo:generate`, still
a one-time/dev-only data-authoring script, never part of `dev`/`build`).
`d3-delaunay` was removed from devDependencies entirely (nothing computes a
Voronoi diagram anymore); `@turf/turf` remains, now used for polygon
simplification, point-in-polygon disambiguation, and distance/area
comparisons instead of boolean set operations. `neighborhoods.geojson` is
now written compact (no pretty-print indentation) to keep it in the same
~270 KB ballpark as before — real municipal boundaries carry far more
vertices per ring than Voronoi cells did, and indentation alone would have
roughly quadrupled the file size.

**Honest caveat, unlike round 2:** real municipal boundaries do not tile
edge-to-edge. There is real unincorporated land between many Chicagoland
towns (and along parts of Chicago's own edge) that now shows as a gap on the
map where round 2's illustrative tessellation had none. That's a more
geographically honest picture, not a regression, and this pipeline makes no
attempt to force those gaps closed.
